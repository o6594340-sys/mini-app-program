...
## Бизнес-контекст и архитектурное ограничение

**Это прототип, не production.** Целевая бизнес-модель — SaaS-подписка для MICE-агентств из РФ.

**Критическое ограничение текущей архитектуры:**
- Данные из админки → localStorage → только на том устройстве
- Участники на своих телефонах → видят дефолтные данные из `data.js`, НЕ то что настроило агентство
- Для реального multi-device использования нужна облачная БД

**Целевая архитектура (переход когда появятся клиенты):**
- Firebase Auth — аккаунты агентств
- Firebase Firestore — данные событий, видны всем участникам
- Firebase Storage — приватные фотобиблиотеки агентств
- ЦА: агентства из РФ → Firebase работает в РФ без VPN

**Фото:** агентства работают с разными направлениями (Камчатка, Париж, Подмосковье и т.д.) — нельзя заранее собрать общую библиотеку. Каждое агентство хранит свои фото. В текущем прототипе фото лежат в `app/photos/` — это временное решение для демо.
